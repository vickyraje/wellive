...
class Search extends Component {
  constructor(props) {
    super(props);
  this.city=localStorage.getItem('place');
  this.states=localStorage.getItem('state');
this.keyword= this.props.match.params.id;
...
let req = new Request(uri, {
  method: 'POST',
  headers: h,
  body: JSON.stringify({'type':this.type,'keyword':this.keyword,'pincode':this.location,'city':this.city,'place':this.place})
});
  ...
  render(){
    const elements = this.state.lists;
     ...
     for (const [indexx, value] of elements.entries()) {
       ...
       const verify=[];
       if(value.verified!=="1"){
       }else {verify.push(<img key={indexx} src={welpverif} alt="verify" className="welptr-ver" />); }
       const trust=[];
       if(value.trusted!=="1"){
       }else {trust.push(<img key={indexx} src={welptrust} alt="trust" className="welptr-trust ml-2" />); }
      ...
      <Col lg="5">
       <Carousel className="shop-carousel">
        ...
        <Col lg="7">
              <h5><a className="mr-3" href={"/Shopview/"+value.name+"/"+value.id}>{value.name}</a>
              {trust}
                    </h5>
...
const elemen= this.state.packages ? this.state.packages :[];
const items1 = []
 ...
 <Col lg="5">
  <Carousel className="shop-carousel">
   ...
   <Col lg="7">
         ...
         {value.year ?  <p><FontAwesomeIcon className="mr-1 theme-txt" icon={faCertificate} /> {value.year} yrs in Business</p>:""}
         {/* <p><FontAwesomeIcon className="mr-1 text-primary" icon={faHourglassHalf} /> 24/7 Availability</p> */}
         { value.offer ? <p className="offers my-1"><img alt="shop-list" src={offer} className="shoplist-icon" /> {value.offer}% offer</p> : ""}
    ...
    const elem= this.state.freelist ? this.state.freelist : [];
const items2 = []
  ...
  <Carousel className="shop-carousel">
   ...
   <Col lg="7">
    ...
    for (const [indexx, value] of elemn.entries()) {
      ...
      const trust=[];
      if(value.trusted!=="1"){
      }else {trust.push(<img key={indexx} alt="trust" src={welptrust} className="welptr-trust ml-2" />); }
    ...
    return (
        ...
        {/* <div className="b-crumb">
          <Container>
            <Row>
              <Col lg="7">
                <h4><span className="bold">{this.keyword} in </span> {this.city}, {this.states}</h4>                          
